...
{
  return qobject_cast<QAbstractItemView*>(widget) ? 5 : 0
         ;
}
...
{
  return qobject_cast<QTableView*>(widget) ? 10 : 0;
}
...
{
  return qobject_cast<QTreeView*>(widget) ? 10 : 0;
}
...
{
  return qobject_cast<QListView*>(widget) ? 10 : 0;
}
...
void ListViewNavigateActionsManagerHelper::currentChanged(const QModelIndex& now, const QModelIndex& old)
{
  ...
  bool isLastRow = currentIndex.row() == currentIndex.model()->rowCount(currentIndex.parent()) - 1;
