...
PrintActionsManagerHelper::~PrintActionsManagerHelper(){ delete printer;}
  ...
  if (itemView) {
    ...
    if (model) {
      ...
      if (printer->printRange() == QPrinter::Selection) {
        indexes = itemView->selectionModel()->selectedIndexes();
        ...
        for (int i = 0; i < model->rowCount(itemView->rootIndex()); ++i)
          for (int g = 0; g < model->columnCount(itemView->rootIndex()); ++g)
            indexes.append(model->index(i, g, itemView->rootIndex()));
      }
      ...
      printer->setDocName(itemView->windowTitle());
