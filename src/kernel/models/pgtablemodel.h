#ifndef PGTABLEMODEL_H
#define PGTABLEMODEL_H
...
Модель для чтения, изменения, добавления, удаления данных sql таблицы.
  ...
  */
class PGTableModelPrivate;
class PGTableModel : public PGQueryModel, public IUndoRedoModel
{
  Q_OBJECT
  Q_INTERFACES(IUndoRedoModel);
  Q_DECLARE_PRIVATE(PGTableModel);
...
protected:
  /*!
    ...
    */
  PGTableModel(PGTableModelPrivate &dd, QObject *parent);
public:

  /*!
    Создает пустую не активную модель
    */
  explicit PGTableModel(QObject *parent = 0);
  /*!
    Уничтожает модель
    */
  virtual ~PGTableModel();
...
protected:
  using PGQueryModel::setQuery;
};

#endif // PGTABLEMODEL_H
