  ...
  if (event->button() == Qt::LeftButton) {
    ...
    if (currentLogicalIndex != -1) {
      ...
      if (d->sortFlag != NoSort) {
        // If multisort and ctrl+shift pressed
        // Если можно сортировать по нескольким столбцам и зажат контрол
        if ((d->sortFlag == MultiSort) && (QApplication::keyboardModifiers() & Qt::ControlModifier)
...
void QtHeaderView::paintEvent(QPaintEvent *e)
{
  Q_D(QtHeaderView);
